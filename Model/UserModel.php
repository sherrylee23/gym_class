...
class UserModel extends BaseModel {
    ...
    public function updateProfile($id, $name, $phone, $password = null) {
        if (!empty($password)) {
            ...
            $stmt = self::db()->prepare("UPDATE users SET full_name = ?, phone_number = ?, password = ? WHERE id = ?");
            return $stmt->execute([$name, $phone, $hashed, $id]);
        } else {
            $stmt = self::db()->prepare("UPDATE users SET full_name = ?, phone_number = ? WHERE id = ?");
    ...
    public function increaseAttempts($id) {
        ...
        if ($newAttempts >= 5) {
            ...
            $stmt = self::db()->prepare("UPDATE users SET failed_attempts = ?, lock_until = ? WHERE id = ?");
            return $stmt->execute([$newAttempts, $lockUntil, $id]);
        } else {
            $stmt = self::db()->prepare("UPDATE users SET failed_attempts = ? WHERE id = ?");
            return $stmt->execute([$newAttempts, $id]);
        }
    }

    public function resetAttempts($id) {
        $stmt = self::db()->prepare("UPDATE users SET failed_attempts = 0, lock_until = NULL WHERE id = ?");
