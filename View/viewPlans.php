...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // 2. Verify the member exists in the JSON data
    $isValidUser = false;
    if ($usersList && !isset($usersList['error'])) {
        foreach ($usersList as $user) {
            if (isset($user['id']) && $user['id'] == $member_id) {
                $isValidUser = true;
                break;
            }
        }
    ...
    // 3. Fallback: if API verification fails, verify directly through UserModel
    if (!$isValidUser) {
        $userModel = new UserModel();
        $user = $userModel->findUserById($member_id);
    ...
    // 4. Make a decision based on the consumed data
    if (!$isValidUser) {
        ...
        $isSuccess = $controller->processCheckout(
            $member_id,
            $_POST['plan_id'],
            $_POST['amount'],
...
<div class="container">
    ...
    <div class="row g-4 justify-content-center">
        <?php foreach ($plans as $plan): ?>
            ...
            <div class="modal fade" id="checkoutModal<?php echo $plan['plan_id']; ?>" tabindex="-1" aria-labelledby="checkoutModalLabel<?php echo $plan['plan_id']; ?>" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content" style="border-radius: 15px;">

                        <div class="modal-header bg-light">
                            <h5 class="modal-title fw-bold" id="checkoutModalLabel<?php echo $plan['plan_id']; ?>">
